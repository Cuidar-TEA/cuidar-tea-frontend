...
const initialState = {
    ...
    profissao: "", especialidade: "", numeroRegistro: "", tipoRegistro: "", ufRegistro: "",
    estado: "", cidade: "", cep: "", logradouro: "", numero: "", bairro: "", complemento: "",
};

export default function FluxoCadastroProfissional() {
  const [step, setStep] = useState(1);
  const { formData, errors, isLoading, isSuccess, handleChange, handleSubmit } = useProfissionalForm(initialState);
      ...
      {isSuccess ? <SuccessStep /> : (
        <>
          ...
          <form onSubmit={handleFormSubmit}>
            {step === 1 && <ProfessionalPersonalInfoStep formData={formData} handleChange={handleChange} errors={errors} />}
            {step === 2 && <ProfessionalDetailsStep formData={formData} handleChange={handleChange} errors={errors} />}
            ...
            <div className="flex justify-between mt-8">
                ...
                <button type="submit" disabled={isLoading} className="px-6 py-2 text-sm font-semibold bg-red-600 text-white rounded-lg shadow hover:bg-red-700 transition-colors duration-300 flex items-center justify-center disabled:opacity-50 w-36 h-10">{isLoading ? ('Carregando...') : (step === 3 ? 'Criar conta' : 'Próximo')}</button>
            </div>
          </form>
        </>
      )}
