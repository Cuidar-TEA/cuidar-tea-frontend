...
export function useCadastroForm(initialState: FormData) {
      ...
      axios.get(`https://viacep.com.br/ws/${cep}/json/`)
        .then(response => {
          if (!response.data.erro) {
            ...
            setErrors((prev: ErrorData) => ({ ...prev, cep: undefined, logradouro: undefined, bairro: undefined, cidade: undefined, estado: undefined }));
          }
        })
        .catch(console.error);
    }
  ...
  const handleSubmit = async (profileType: 'familia' | 'profissional') => {
    if (!validateAll()) {
      alert("Existem erros no formulário. Por favor, verifique os campos em vermelho.");
      return;
    }
    ...
    if (profileType === 'familia') {
      ...
      // Futuramente, a lógica para o payload do profissional
    }
    ...
    try {
      ...
      if (err.response?.status === 409) { setApiError(err.response.data.message); }
      else if (err.response?.status === 400) { setApiError("Dados inválidos. Verifique os campos e tente novamente."); }
      else { setApiError("Ocorreu um erro inesperado. Tente novamente mais tarde."); }
    } finally {
      setIsLoading(false);
    }
  };
